...
def get_num_unique_mut(tree, num_mut):  # recursion is the best, dawg
    if len(tree.children) > 0:
        for child in tree.children:
            num_mut = get_num_unique_mut(child, num_mut + 1)
    return num_mut

def get_num_unique_mut_threshold(tree, num_mut, threshold):  # recursion is the best, dawg
    if len(tree.children) > 0:
        for child in tree.children:
            if child.get_total_size()>=threshold:
                num_mut = get_num_unique_mut_threshold(child, num_mut + 1, threshold)
    return num_mut

def findLocalOptima(x, t, input_type, break_mult = 2): # x = cell count over time, t = time vector in days; break_mult = fold change relative to initial tumor size when we stop looking for pseudoprogression
    n = len(x) # number of days
    x_prime = np.zeros(n-2)
    startingsize = x[0]
    starting_data = {'time': t[0], 'cell count': [int(startingsize)], 'fold change': [round(x[0]/startingsize,2)], 'type': ['initiation'], 'slope': [0]}
    optima_df = pd.DataFrame(data=starting_data)
    nrows = 1
    for i in range(1,n-1):
        if input_type=='tumor' and x[i-1] >= break_mult*startingsize: # if tumor cell count is over our acceptability threshhold (default 2x the initial tumor size),
            prev_x = optima_df['cell count'].loc[nrows-1]
            prev_t = optima_df['time'].loc[nrows-1]
            approx_slope = (x[i-1] - prev_x)/(t[i-1] - prev_t)
            break_data = pd.DataFrame({'time': [t[i - 1]], 'cell count': [int(x[i-1])], 'fold change': [round(x[i - 1]/startingsize,2)], 'type': ['progression limit'], 'slope': [approx_slope]})
            optima_df = pd.concat([optima_df, break_data], ignore_index=True)
            break # then stop searching.
        x_prime[i-1] = (x[i+1] - x[i-1])/2 # central difference formula for dx/dt
        if (i>1)&(i<n-1):
            if (x_prime[i-2]>0)&(x_prime[i-1]<=0):
                prev_x = optima_df['cell count'].loc[nrows - 1]
                prev_t = optima_df['time'].loc[nrows - 1]
                approx_slope = (x[i - 1] - prev_x) / (t[i - 1] - prev_t)
                peak_data = pd.DataFrame({'time': [t[i-1]], 'cell count': [int(x[i-1])], 'fold change': [round(x[i - 1]/startingsize,2)], 'type': ['max'], 'slope': [approx_slope]})
                optima_df = pd.concat([optima_df, peak_data], ignore_index=True)
                nrows += 1
            elif (x_prime[i-2]<0)&(x_prime[i-1]>=0):
                prev_x = optima_df['cell count'].loc[nrows - 1]
                prev_t = optima_df['time'].loc[nrows - 1]
                approx_slope = (x[i - 1] - prev_x) / (t[i - 1] - prev_t)
                peak_data = pd.DataFrame({'time': [t[i - 1]], 'cell count': [int(x[i-1])], 'fold change': [round(x[i - 1]/startingsize,2)], 'type': ['min'], 'slope': [approx_slope]})
                optima_df = pd.concat([optima_df, peak_data], ignore_index=True)
                nrows += 1
    return optima_df


def pruneOptima(df, size_sensitivity = 0.25):
    num_entries = len(df['time'])
    if df['type'].loc[num_entries-1]=='progression limit':
        num_entries -= 1
    idx = 1 # start by excluding the initial tumor size
    prev_magnitude = df['fold change'].loc[0]
    while idx < num_entries:
        cur_magnitude = df['fold change'].loc[idx]
        # check that this is a large enough fluctuation to merit inclusion
        if abs(cur_magnitude-prev_magnitude) < size_sensitivity: # drop this optimum if it's too small in magnitude
            df = df.drop([idx])
        else:
            prev_magnitude = np.copy(cur_magnitude)
        idx += 1
    return df.reset_index(drop=True)

def getMax(df):
    maxdf = df.loc[df['type']=='max']
    return maxdf.reset_index(drop=True)

def getPeakDelays(tumor_maxdf, tcell_maxdf):
    n_tumor_peaks = len(tumor_maxdf)
    n_tcell_peaks = len(tcell_maxdf)
    peak_delays = []
    if n_tcell_peaks==0 or n_tumor_peaks==0:
        return [] # return an empty delay
    tu_t = 0
    tu_idx = 0 # tumor index
    ef_idx = -1 # effector cell index
    while tu_idx<n_tumor_peaks and ef_idx<n_tcell_peaks:
        tu_t = tumor_maxdf['time'].loc[tu_idx] # find time of this tumor peak
        ef_peak = tcell_maxdf.loc[tcell_maxdf['time']>tu_t] # collect all t cell peaks after this tumor peak
        if len(ef_peak)==0: # if there are none, then break the loop
            break
        else:
            tu_idx += 1 # increment tumor peak index
            all_relevant_idx = [j for j in ef_peak.index if j>ef_idx]
            if len(all_relevant_idx)>0:
                ef_idx = all_relevant_idx[0] # take the next peak in effector cells as the matching one
                delay = ef_peak['time'].loc[ef_idx] - tu_t # and record the delay between t cell and tumor peaks
                peak_delays.append(delay)
            else:
                break
    return np.array(peak_delays)

def getPseudoprogression(LTR, TMB_over_time, effector_over_time, t):
    # tumor_vol_over_time = [estimate_lesion_diameter(i) for i in TMB_over_time]
    early_threshold = 12 * 7  # consider this "early" pseudoprogression if within first 12 weeks
    timing_dict = {'early': 0, 'late': 0}
    early_events = 0
    late_events = 0
    num_pseudoprogression_events = 0
    initial_Tcell_slope = -1
    time_to_first_Tcell_peak = -1
    avg_delay = -1
    time_to_first_psp = -1
    if LTR != 'No Response':
        prelim_tumor_optima_df = findLocalOptima(TMB_over_time, t, 'tumor')
        tumor_optima_df = pruneOptima(prelim_tumor_optima_df)
        num_entries = len(tumor_optima_df['type'])
        subset_of_effectors = False # flag for whether we need to subset effectors
        if num_entries > 1:  # if == 1, there is only the initial point recorded. Only care about T cells if >1
            last_entry = tumor_optima_df['type'].loc[num_entries - 1]
            if last_entry == 'progression limit':
                if num_entries - 1 > 1:  # if there is still at least 1 optimum for us to view,
                    end_t = tumor_optima_df['time'].loc[num_entries - 1]
                    subset_of_effectors = True
                    check_tcell = True
                else:
                    check_tcell = False
            else:
            ...
            check_tcell = False
        if check_tcell:
            if subset_of_effectors:
                t_subset = [i for i in t if i<end_t]
                eff_subset = [effector_over_time[i] for i in range(len(effector_over_time)) if t[i]<end_t]
                prelim_tcell_optima_df = findLocalOptima(eff_subset, t_subset, 'tcell')
            else:
                prelim_tcell_optima_df = findLocalOptima(effector_over_time, t, 'tcell')
            tcell_optima_df = pruneOptima(prelim_tcell_optima_df)
            tcell_max_df = getMax(tcell_optima_df)
            tumor_max_df = getMax(tumor_optima_df)
            num_pseudoprogression_events = len(tumor_max_df)
            if num_pseudoprogression_events>0:
                max_times = tumor_max_df['time'].to_numpy()
                time_to_first_psp = max_times[0]  # record first psp event, in days
                for mt in max_times:
                    if mt < early_threshold:
                        timing_dict['early'] += 1
                    else:
                        timing_dict['late'] += 1
                early_events = timing_dict['early']
                late_events = timing_dict['late']
                if len(tcell_max_df) > 0:
                    initial_Tcell_slope = tcell_max_df['slope'].loc[0]  # slope to first optimum
                    time_to_first_Tcell_peak = tcell_max_df['time'].loc[0]
                    delays = getPeakDelays(tumor_max_df, tcell_max_df)
                    if len(delays) > 0:
                        avg_delay = np.mean(delays)
    pseud = 'No PsP'
    if num_pseudoprogression_events > 0:
        pseud = 'PsP'
    return [early_events, late_events, num_pseudoprogression_events, initial_Tcell_slope, time_to_first_Tcell_peak, avg_delay, time_to_first_psp]


# def alignOptima(tumor_df, tcell_df): # these must contain only maxima!!!
#     idx = tumor_df['time'].loc[0]
#     nc = min(len(tumor_df), len(tcell_df))
#     while nc > 0:
#
#     return delays

def check_imRECIST(TMB_over_time, t, min_increment = 4*7):
    Tmax = t[-1]
    ...
    return [time_to_RECIST_PD, RECIST_pdflag, time_to_CR, time_to_imRECIST_iCPD, imRECIST_response_tmax]

def check_iRECIST(TMB_over_time, t, min_increment = 4*7):
    Tmax = t[-1]
    time_to_RECIST_PD = Tmax
    RECIST_pdflag = False
    ...
    return [time_to_RECIST_PD, RECIST_pdflag, time_to_iRECIST_iCPD, iRECIST_response_tmax]

def check_irRECIST(TMB_over_time, t, min_increment = 4*7):
    Tmax = t[-1]
    time_to_RECIST_PD = Tmax
    RECIST_pdflag = False
    ...
    return [time_to_RECIST_PD, RECIST_pdflag, time_to_irRECIST_iCPD, irRECIST_response_tmax]

def simulate_Colle_analysis(tumor_size_over_time, t):
    tumor_pop = [tumor_size_over_time[0]]
    t_subset = [t[0]]
    first_eval_t = int(3*30.4368)
    tumor_pop.append(tumor_size_over_time[first_eval_t])
    t_subset.append(first_eval_t)
    first_eval = getResponse(tumor_pop, t_subset)
    if first_eval=='PD':
        # evaluate with iRECIST: check 4-8 weeks later. We average to 6.
        follow_up_time = first_eval_t + 6*7
        tumor_pop.append(tumor_size_over_time[follow_up_time])
        t_subset.append(follow_up_time)
        follow_up = getResponse(tumor_pop, t_subset)
        if follow_up=='PD':
            result = 'early PD'
        else:
            result = 'early PsP'
    else:
        # Continue evaluating every 6-10 weeks; we average to 8.
        cur_t = first_eval_t + 8*7
        result = 'DR'
        while cur_t < t[-1]: # as long as there is time left in the study,
            tumor_pop.append(tumor_size_over_time[cur_t])
            t_subset.append(cur_t)
            cur_resp = getResponse(tumor_pop, t_subset)
            if cur_resp=='PD':
                check_time = cur_t + 6*7
                if check_time < t[-1]:
                    tumor_pop.append(tumor_size_over_time[check_time])
                    t_subset.append(check_time)
                    check_resp = getResponse(tumor_pop, t_subset)
                    if check_resp=='PD':
                        result = 'late PD'
                        break
                    else:
                ...
                cur_t += 8*7 # increment the current time and continue
    return result
...
def getPseudoprogression_deprecated(tumor_size_over_time, t):
    epsilon = 1e-6
    sum_lesion_diams = [estimate_lesion_diameter(tumor_size_over_time[iii]) for iii in range(len(t))]
